...
// Calculate 2 vectors Nt and Nb which correspond to x and z axis
// when N is considered as a y axix
...
// Loop over all lights in the scene and sum their contribution up We also apply the lambert cosine law here though we haven't explained yet what this means. 
                ...
                for (uint32_t n = 0; n < maxN; ++n) {
                    ...
                    indirectLighting += r1 * castRay(bvh, 
                                                    reflectionRayOrig + sampleWorld, 
                                                    sampleWorld, 
                                                    objects, 
                                                    lights, 
                                                    options, 
                ...
                indirectLighting /= (float)maxN;

                Vec3f directLighting = lightAmt * hitObject->evalDiffuseColor(st) * hitObject->Kd + specularColor * hitObject->Ks; 
                hitColor = (directLighting / M_PI + 2 * indirectLighting) * hitObject->albedo;
...
bool addLightRoom(std::vector<std::shared_ptr<Object>> &objects) {
    ...
    //left1->albedo = Vec3f(0.225, 0.144, 0.144);
    //left2->albedo = Vec3f(0.225, 0.144, 0.144);
    ...
    //right1->albedo = Vec3f(0.225, 0.144, 0.144);
    //right2->albedo = Vec3f(0.225, 0.144, 0.144);
    ...
    //floor1->albedo = Vec3f(0.225, 0.144, 0.144);
    //floor2->albedo = Vec3f(0.225, 0.144, 0.144);
    ...
    //back1->albedo = Vec3f(0.225, 0.144, 0.144);
    //back2->albedo = Vec3f(0.225, 0.144, 0.144);
...
{ 
    ...
    //sph1->albedo = Vec3f(0.225, 0.144, 0.144);

    Sphere *sph2 = new Sphere(Vec3f(3, -1, -5), 1.5);
    //sph2->ior = 8;
    sph2->diffuseColor = Vec3f(0.6, 0.7, 0.2);
    sph2->materialType = DIFFUSE_AND_GLOSSY;
    //sph2->albedo = Vec3f(0.225, 0.144, 0.144);
    ...
    std::vector<Vec3f> vertices; 
    std::vector<int> vertexIndex; 
    // importMesh("../model/dragon_vrip_res4.ply", vertices, vertexIndex);
